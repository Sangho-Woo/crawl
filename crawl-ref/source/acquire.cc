...
{
    vector<object_class_type> acquire_classes;

    if (you.species != SP_FELID)
    {
        acquire_classes.push_back(OBJ_WEAPONS);
        acquire_classes.push_back(OBJ_ARMOUR);
        acquire_classes.push_back(OBJ_STAVES);
    }

    acquire_classes.push_back(OBJ_JEWELLERY);
    acquire_classes.push_back(OBJ_BOOKS);
	acquire_classes.push_back(OBJ_MISCELLANY);
	acquire_classes.push_bacK(OBJ_GOLD);

    if (!you_foodless(false))
        acquire_classes.push_back(OBJ_FOOD);

    CrawlVector &acq_items = you.props[ACQUIRE_ITEMS_KEY].get_vector();
    acq_items.empty();

    // Generate item in all classes
    for (auto obj_type : acquire_classes)
    {
